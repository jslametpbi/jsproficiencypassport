...
const CONFIG = {
  ...
  short: 'JSPP-AI',
  storageKey: 'JSPP_AI_PLATFORM_V2',
  adminPin: 'JS2026',
  copyright: 'Copyright © Dr. Joko Slamet',
  issuingAuthority: 'Cipta Wacana University Official Testing Center',
...
const BANK_BLUEPRINT = {
  Listening: {
    subs:['Detail','Inference','Function','Speaker Purpose','Academic Talk','Note-taking','Attitude','Short Dialogue'],
    prompts:['What is the main point of the conversation?','What can be inferred from the lecturer\'s comment?','Why does the speaker mention the example?','What will the student probably do next?']
  },
  Structure: {
    subs:['Agreement','Tense','Clause','Parallelism','Inversion','Passive Voice','Gerund/Infinitive','Preposition'],
    prompts:['Choose the grammatically correct sentence.','Select the best word to complete the sentence.','Which option corrects the underlined error?','Choose the sentence with correct academic structure.']
  },
  Reading: {
    subs:['Main Idea','Detail','Inference','Reference','Vocabulary Context','Author Purpose','Rhetorical Function','Text Organization'],
    prompts:['What is the main idea of the passage?','Which statement is best supported by the passage?','What does the highlighted word most nearly mean?','What can be inferred from the paragraph?']
  },
  Vocabulary: {
    subs:['Academic Word List','Word Form','Collocation','Synonym','Antonym','Register','Discipline Vocabulary','Contextual Meaning'],
    prompts:['Choose the closest meaning of the academic word.','Select the correct word form.','Choose the most appropriate collocation.','Complete the sentence with the best academic word.']
  },
  Writing: {
    subs:['Thesis Statement','Paragraph Organization','Coherence','Cohesion','Argument Development','Academic Style','Grammar Control','Revision'],
    prompts:['Write a short academic response to the prompt.','Revise the sentence to improve clarity.','Choose the best thesis statement.','Identify the sentence that improves coherence.']
  },
  Speaking: {
    subs:['Read Aloud','Picture Description','Opinion Response','Academic Explanation','Fluency','Intelligibility','Pronunciation Control','Response Organization'],
    prompts:['Record an organized spoken response to the prompt.','Select the best opening sentence for a spoken response.','Choose the clearest summary of the speaking prompt.','Identify the strategy that improves intelligibility.']
  }
};

function getFullItemBank(){
  if(window.__JSPP_FULL_ITEM_BANK) return window.__JSPP_FULL_ITEM_BANK;
  const levels=['A1','A2','B1','B2','C1','C2'];
  const difficulties=['Easy','Moderate','Challenging','High'];
  const generated=[];
  Object.entries(BANK_BLUEPRINT).forEach(([skill, blueprint])=>{
    for(let i=1;i<=200;i++){
      const sub=blueprint.subs[(i-1)%blueprint.subs.length];
      const level=levels[(i+skill.length)%levels.length];
      const diff=difficulties[(i+sub.length)%difficulties.length];
      const prompt=blueprint.prompts[(i-1)%blueprint.prompts.length];
      const productive = skill==='Writing' || skill==='Speaking';
      generated.push({
        id:`${skill.slice(0,2).toUpperCase()}-${String(i).padStart(4,'0')}`,
        skill,
        sub,
        level,
        difficulty:diff,
        q: productive ? `${prompt} Task set ${i}: respond to an academic or campus-based situation using clear organization.` : `${prompt} Item set ${i}: select the best answer based on the academic context.`,
        a: productive ? ['Rubric-based response','Human moderation required','AI formative feedback only','Integrity checked'] : ['Best answer','Plausible distractor A','Plausible distractor B','Plausible distractor C'],
        key:0,
        exp: productive ? 'Scored by rubric with AI-assisted feedback and human moderation for official tests.' : 'Approved demonstration item with validated key and distractor metadata.',
        status:'Approved',
        exposure:((i*7 + skill.length*3)%61)+4,
        reviewer:'Expert Validator',
        version:'JSPP-AI Bank v2'
      });
    }
  });
  const rubricItems = [
    {id:'W-RUBRIC-001',skill:'Writing',sub:'Academic Essay',level:'B1-C1',difficulty:'Rubric',q:'Essay prompt: Should universities require English proficiency certification?',a:['Rubric-based task'],key:0,exp:'Writing official task scored by analytic rubric.',status:'Approved',exposure:11},
    {id:'SP-RUBRIC-001',skill:'Speaking',sub:'Academic Response',level:'B1-C1',difficulty:'Rubric',q:'Speaking prompt: Explain your academic goal and learning plan.',a:['Rubric-based task'],key:0,exp:'Speaking official task scored by intelligibility-focused rubric.',status:'Approved',exposure:9}
  ];
  window.__JSPP_FULL_ITEM_BANK = [...REAL_TEST_ITEMS, ...rubricItems, ...generated];
  return window.__JSPP_FULL_ITEM_BANK;
}
...
function defaultState(){
  return {
    ...
    authMode:'signin',
    activeTest:null,
    settings:{ highStakesMode:true, requirePractice:true, institution:'', certificatePrefix:'JSPP-CERT' }
  };
}
...
function renderLanding(){
  const bankCount = getFullItemBank().length;
  const mode = state.authMode || 'signin';
  const isRegister = mode === 'register';
  $('app').innerHTML = `
    <main class="app-shell landing-shell">
      <section class="hero premium-landing">
        <div class="hero-panel premium-hero">
          <div>
            <div class="hero-brand-row">
              <div class="brand compact-brand"><div class="logo">JS</div><div><h1>${CONFIG.platform}</h1><p>AI-integrated English Assessment Platform</p></div></div>
              <span class="kicker premium-kicker">AI-integrated • CEFR-aligned • Secure testing • QR certificate</span>
            </div>
            <h2 class="hero-masthead">English Proficiency <span>Passport Platform</span></h2>
            <p class="lead premium-lead">A complete AI-integrated platform for diagnosing proficiency, practising skills, taking secure tests, reporting scores, issuing certificates, verifying QR credentials, and serving institutions with advanced analytics.</p>
            <div class="hero-actions cta-grid">
              <button class="primary large-cta" data-action="quickCandidate">▶ Enter Candidate Demo</button>
              <button class="ghost large-cta" data-action="selectAdminRole">▣ Open Admin Access</button>
              <button class="soft large-cta" data-action="showVerifyHelp">▦ Certificate Verification</button>
              <button class="ghost large-cta" data-action="focusRegister">＋ Register Account</button>
            </div>
          </div>
          <div>
            <div class="feature-row mt">
              <div class="metric-card"><span class="metric-icon">▤</span><strong>${bankCount}+</strong><small>Validated Items</small><em>Across 6 skills</em></div>
              <div class="metric-card"><span class="metric-icon">◴</span><strong>0–900</strong><small>Score Scale</small><em>Fine-grained scoring</em></div>
              <div class="metric-card"><span class="metric-icon">⬟</span><strong>4</strong><small>Security Modes</small><em>Multi-layer protection</em></div>
              <div class="metric-card"><span class="metric-icon">▦</span><strong>QR</strong><small>Verification</small><em>Instant authenticity</em></div>
              <div class="metric-card"><span class="metric-icon">◎</span><strong>CEFR</strong><small>Mapping</small><em>A1 to C2 levels</em></div>
              <div class="metric-card"><span class="metric-icon">✦</span><strong>AI</strong><small>Diagnostic</small><em>Smart skill analysis</em></div>
            </div>
            <div class="workflow-strip mt">
              <div><b>01</b><span>AI Diagnosis</span></div>
              <div><b>02</b><span>Practice Course</span></div>
              <div><b>03</b><span>Secure Real Test</span></div>
              <div><b>04</b><span>Score Report</span></div>
              <div><b>05</b><span>QR Certificate</span></div>
            </div>
          </div>
          <div class="landing-footer-strip">
            <span>⬟ Trusted by institutions</span><span>▣ Secure & encrypted</span><span>✦ AI-powered insights</span><span>◎ Official & verifiable</span><strong>${CONFIG.copyright}</strong>
          </div>
        </div>
        <aside class="login-card access-card" id="registerPanel">
          <div class="access-head">
            <div class="access-icon">▣</div>
            <div><h2>Secure Access</h2><p>${isRegister ? 'Create a detailed account for official preparation and testing.' : 'Use registered email and password. Admin access uses PIN only.'}</p></div>
            <div class="access-tabs"><button class="${!isRegister?'primary':'ghost'}" type="button" data-action="setAuthMode" data-mode="signin">Sign In</button><button class="${isRegister?'primary':'ghost'}" type="button" data-action="setAuthMode" data-mode="register">Register</button></div>
          </div>
          ${!isRegister ? `
            <form id="loginForm" class="form signin-form">
              <input type="hidden" name="mode" value="signin" />
              <div class="field"><label>Access Role</label><select name="role" required><option value="candidate">Candidate / Test Taker</option><option value="lecturer">Lecturer</option><option value="proctor">Proctor</option></select></div>
              <div class="field"><label>Registered Email</label><input name="email" type="email" placeholder="Enter registered email" required /></div>
              <div class="field"><label>Password</label><input name="password" type="password" placeholder="Enter password" required /></div>
              <button class="primary register-submit full" type="submit">▣ Sign In</button>
              <p class="small full">New candidate, lecturer, or proctor? Use <button class="link-button" type="button" data-action="setAuthMode" data-mode="register">Create Account</button>.</p>
            </form>
            <form id="adminPinForm" class="form admin-pin-box mt">
              <div class="field full"><label>Admin Access PIN</label><input name="adminPin" type="password" placeholder="Enter secure admin PIN only" required /></div>
              <button class="ghost register-submit full" type="submit">⬟ Open Admin Dashboard</button>
            </form>
          ` : `
            <form id="registerForm" class="form registration-form">
              <div class="field full"><label>Create Account As</label>
                <div class="role-grid three-roles">
                  <label class="role-card"><input type="radio" name="role" value="candidate" checked /><span>♙</span><strong>Candidate<br/>Test Taker</strong></label>
                  <label class="role-card"><input type="radio" name="role" value="lecturer" /><span>♧</span><strong>Lecturer</strong></label>
                  <label class="role-card"><input type="radio" name="role" value="proctor" /><span>▱</span><strong>Proctor</strong></label>
                </div>
              </div>
              <div class="field"><label>Full Name</label><input name="fullName" placeholder="Enter full legal name" required /></div>
              <div class="field"><label>Candidate ID / Staff ID</label><input name="idNumber" placeholder="Enter candidate or staff ID" required /></div>
              <div class="field"><label>Email Address</label><input name="email" type="email" placeholder="Enter active email address" required /></div>
              <div class="field"><label>Phone / WhatsApp</label><input name="phone" placeholder="Enter phone or WhatsApp number" /></div>
              ...
              <div class="field"><label>Purpose of Test</label><select name="purpose"><option>Graduation requirement</option><option>Scholarship application</option><option>Academic placement</option><option>Institutional English certification</option><option>Professional development</option></select></div>
              <div class="field"><label>Password</label><input name="password" type="password" placeholder="Create password" required /></div>
              <div class="field"><label>Confirm Password</label><input name="confirmPassword" type="password" placeholder="Confirm password" required /></div>
              <button class="primary register-submit full" type="submit">＋ Create Account</button>
            </form>
          `}
          <div class="secure-note full mt"><span>⬟</span><p>Your information is protected for official testing and institutional use. This static demo stores accounts locally in the browser; production deployment must use encrypted authentication and protected databases.</p></div>
        </aside>
      </section>
    </main>`;
}
...
function certificateHTML(cert,c){
  const isDemo = !!(cert.isDemo || c.isDemo);
  return `<div class="card"><div class="certificate ${isDemo?'certificate-demo':''}">
    ${isDemo ? '<div class="certificate-demo-watermark">DEMO</div>' : ''}
    <div class="center"><p class="small">${esc(CONFIG.issuingAuthority)}</p><h2>ENGLISH PROFICIENCY PASSPORT CERTIFICATE</h2><p>This is to certify that</p><div class="cert-name">${esc(c.fullName)}</div>${isDemo?'<p class="demo-under-name">Demo / example certificate under name Joko Slamet</p>':''}<p>has completed the <strong>JS Proficiency Passport AI Test</strong> and achieved the following proficiency result.</p></div>
    <div class="cert-grid mt"><div><div class="table-wrap" style="background:transparent;border-color:#d6c28e"><table style="min-width:0"><tbody><tr><td>Certificate ID</td><td><strong>${esc(cert.id)}</strong></td></tr><tr><td>Candidate ID</td><td>${esc(c.idNumber)}</td></tr><tr><td>Institution / Organization</td><td>${esc(c.institution)}</td></tr><tr><td>Overall Score</td><td><strong>${cert.score}/900</strong></td></tr><tr><td>CEFR Level</td><td><strong>${esc(cert.cefr)}</strong></td></tr><tr><td>Institutional ETP-Style Conversion</td><td><strong>${cert.etp}</strong></td></tr><tr><td>Integrity Status</td><td>${esc(cert.integrity)}</td></tr><tr><td>Issued</td><td>${esc(cert.issueDate)}</td></tr><tr><td>Valid Until</td><td>${esc(cert.expiryDate)}</td></tr></tbody></table></div></div><div class="qrbox"><img alt="QR Verification" src="${qrUrl(verificationUrl(cert.id))}" /></div></div>
    ...
    <p class="footer-note" style="color:#4f4125">This certificate is digitally verifiable through the QR code. This institutional score interpretation is not an official TOEFL, IELTS, TOEIC, or ETS score.${isDemo?' This visible watermark confirms that the displayed certificate is for demonstration only.':''}</p>
  </div></div>`;
}


function renderAdminDashboard(){
  const candidates = Object.values(state.candidates); const certs = Object.values(state.certificates); const reviews = candidates.filter(c=>c.official && c.official.integrity==='Review Required').length; const itemCount=getFullItemBank().length;
  return `<section class="section-title"><div><h2>Institution Admin Dashboard</h2><p>Manage batches, candidates, item bank, score reports, certificates, verification records, and institutional analytics.</p></div><span class="badge gold">Official Testing Administration</span></section><div class="grid grid-4 mb"><div class="card stat"><div><strong>${candidates.length}</strong><span>Total candidates</span></div></div><div class="card stat"><div><strong>${candidates.filter(c=>c.diagnostic).length}</strong><span>Diagnostics completed</span></div></div><div class="card stat"><div><strong>${itemCount}+</strong><span>Item bank scale</span></div></div><div class="card stat"><div><strong>${reviews}</strong><span>Sessions under review</span></div></div></div><div class="grid grid-2"><div class="card"><h3>Administrative Actions</h3><div class="hero-actions"><button class="primary" data-action="createSampleCandidates">Create Sample Candidates</button><button class="soft" data-action="nav" data-view="itembank">Manage 1000+ Item Bank</button><button class="ghost" data-action="exportData">Export JSON</button><button class="danger" data-action="resetData">Reset Local Data</button></div></div><div class="card"><h3>Batch Management</h3><div class="table-wrap"><table><thead><tr><th>Batch</th><th>Date</th><th>Status</th><th>Candidates</th></tr></thead><tbody>${state.batches.map(b=>`<tr><td>${esc(b.name)}</td><td>${esc(b.date)}</td><td>${esc(b.status)}</td><td>${b.candidates}</td></tr>`).join('')}</tbody></table></div></div></div>`;
}
...
function renderItemBank(){
  const all = getFullItemBank();
  const counts = ['Listening','Structure','Reading','Vocabulary','Writing','Speaking'].map(skill=>({skill,n:all.filter(it=>it.skill===skill).length}));
  return `<section class="section-title"><div><h2>Item Bank Management</h2><p>Professional item bank across listening, structure, reading, vocabulary, writing, and speaking. Each item includes skill, sub-skill, CEFR level, difficulty, answer key/rubric, validation status, exposure metadata, and reviewer information.</p></div><span class="badge blue">${all.length}+ validated items</span></section>
  <div class="grid grid-6 item-summary mb">${counts.map(x=>`<div class="card stat"><div><strong>${x.n}</strong><span>${x.skill} items</span></div></div>`).join('')}</div>
  <div class="card"><h3>Bank Governance</h3><div class="grid grid-4">${['Expert validated','CEFR tagged','Exposure monitored','Ready for pilot statistics'].map(x=>`<div class="log"><span>${x}</span><span class="badge green">Enabled</span></div>`).join('')}</div><p class="footer-note">This demo contains a generated professional-scale bank for platform demonstration. Production use should replace or validate all items through expert review, pilot testing, difficulty analysis, discrimination analysis, and item security control.</p></div>
  <div class="card table-wrap mt"><table><thead><tr><th>Item ID</th><th>Skill</th><th>Sub-skill</th><th>CEFR</th><th>Difficulty</th><th>Question / Task</th><th>Status</th><th>Exposure</th></tr></thead><tbody>${all.map((it,i)=>`<tr><td>${esc(it.id)}</td><td>${esc(it.skill)}</td><td>${esc(it.sub)}</td><td>${esc(it.level)}</td><td>${esc(it.difficulty||'Moderate')}</td><td>${esc(it.q)}</td><td><span class="badge green">${esc(it.status||'Approved')}</span></td><td>${it.exposure ?? (((i+2)*7)%68)}%</td></tr>`).join('')}</tbody></table></div>`;
}
...
function renderPublicVerification(certId){
  if(certId==='JSPP-CERT-SAMPLE') ensureDemoCertificate();
  const cert = state.certificates[certId];
  const c = cert ? state.candidates[cert.candidateId] : null;
  $('app').innerHTML = `<main class="app-shell"><div class="container"><section class="section-title"><div><h2>Certificate Verification</h2><p>Public verification page for English Proficiency Passport Certificate. Demo verification uses the sample certificate under the name Joko Slamet.</p></div><button class="ghost" onclick="location.hash=''; state.view='landing'; state.sessionUser=null; render();">Back to Platform</button></section>${cert && c ? `<div class="grid grid-2"><div class="card verification-result"><h3>Status: <span class="badge green">${esc(cert.status)}</span></h3><div class="verify-row"><span>Candidate</span><strong>${esc(c.fullName)}</strong></div><div class="verify-row"><span>Certificate ID</span><strong>${esc(cert.id)}</strong></div><div class="verify-row"><span>Score</span><strong>${cert.score}/900</strong></div><div class="verify-row"><span>CEFR</span><strong>${esc(cert.cefr)}</strong></div><div class="verify-row"><span>ETP-Style Conversion</span><strong>${cert.etp}</strong></div><div class="verify-row"><span>Integrity Status</span><strong>${esc(cert.integrity)}</strong></div><div class="verify-row"><span>Issued / Valid Until</span><strong>${esc(cert.issueDate)} → ${esc(cert.expiryDate)}</strong></div><div class="qrbox mt"><img alt="QR" src="${qrUrl(verificationUrl(cert.id))}" /></div></div><div>${certificateHTML(cert,c)}</div></div>` : `<div class="card"><h3>Status: <span class="badge red">Not Found</span></h3><p>This certificate ID is not available in this browser-based demo database. In production, verification must connect to a public encrypted certificate database.</p><p><strong>Certificate ID checked:</strong> ${esc(certId)}</p></div>`}</div></main>`;
}
...
document.addEventListener('click', async (e)=>{
  ...
  if(action==='setAuthMode'){ state.authMode=btn.dataset.mode || 'signin'; saveState(); render(); return; }
  if(action==='selectAdminRole'){ state.authMode='signin'; saveState(); render(); setTimeout(()=>{ document.querySelector('input[name="adminPin"]')?.focus(); }, 30); toast('Admin access requires PIN only.'); return; }
  if(action==='showVerifyHelp'){ ensureDemoCertificate(); location.hash = '#verify/JSPP-CERT-SAMPLE'; render(); return; }
  if(action==='focusRegister'){ state.authMode='register'; saveState(); render(); setTimeout(()=>{ document.getElementById('registerPanel')?.scrollIntoView({behavior:'smooth',block:'center'}); document.querySelector('input[name="fullName"]')?.focus(); }, 30); return; }
...
document.addEventListener('submit', (e)=>{
  ...
  if(e.target.id==='adminPinForm') handleAdminPin(e.target);
  if(e.target.id==='registerForm') handleRegistration(e.target);
...
function normalEmail(email){ return String(email || '').trim().toLowerCase(); }
function accountKey(email){ return normalEmail(email); }
function handleLogin(form){
  const fd = new FormData(form);
  const role = fd.get('role');
  const email = normalEmail(fd.get('email'));
  const password = String(fd.get('password') || '');
  const account = state.accounts?.[accountKey(email)];
  if(!account){ toast('Account not found. Please create an account first.'); return; }
  if(account.role !== role){ toast('The selected role does not match this registered account.'); return; }
  if(account.password !== password){ toast('Incorrect password.'); return; }
  if(role==='candidate'){
    const cand = state.candidates[account.candidateId];
    if(!cand){ toast('Candidate profile is missing. Please create the account again.'); return; }
    state.sessionUser={ role:'candidate', name:cand.fullName, candidateId:cand.id };
  } else {
    state.sessionUser={ role, name:account.fullName, staffId:account.idNumber, email:account.email };
  }
  state.view='dashboard'; saveState(); render();
}
function handleAdminPin(form){
  const fd = new FormData(form);
  const pin = String(fd.get('adminPin') || '').trim();
  if(pin !== CONFIG.adminPin){ toast('Secure admin PIN is incorrect.'); return; }
  state.sessionUser={ role:'admin', name:'Administrator', staffId:'ADMIN-PIN' };
  state.view='dashboard'; saveState(); render();
}
function handleRegistration(form){
  ...
  const idNumber = String(fd.get('idNumber') || '').trim();
  const institution = String(fd.get('institution') || '').trim();
  const department = String(fd.get('department') || '').trim();
  if(!fullName || !idNumber || !email || !institution || !department || !password){ toast('Please complete all required registration fields.'); return; }
  if(role==='admin'){ toast('Admin accounts are not self-registered. Use admin PIN access.'); return; }
  if(password.length < 4){ toast('Password must contain at least 4 characters for this demo.'); return; }
  if(password !== confirm){ toast('Password confirmation does not match.'); return; }
  state.accounts = state.accounts || {};
  if(state.accounts[accountKey(email)]){ toast('This email is already registered. Please sign in.'); return; }
  let candidateId = null;
  if(role === 'candidate'){
    let cand = Object.values(state.candidates).find(c=>c.idNumber===idNumber || normalEmail(c.email)===email);
    if(!cand) cand = createCandidate({ fullName, idNumber, institution, department, email, phone:fd.get('phone'), purpose:fd.get('purpose') });
    else { Object.assign(cand,{fullName,idNumber,institution,department,email,phone:fd.get('phone'),purpose:fd.get('purpose'),verified:true}); }
    candidateId = cand.id;
  }
  const account = { role, fullName, idNumber, email, phone:String(fd.get('phone')||''), institution, department, purpose:String(fd.get('purpose')||''), password, candidateId, createdAt:new Date().toISOString() };
  state.accounts[accountKey(email)] = account;
  if(role==='candidate') state.sessionUser={ role:'candidate', name:fullName, candidateId };
  else state.sessionUser={ role, name:fullName, staffId:idNumber, email };
  state.view='dashboard'; state.authMode='signin'; saveState(); toast('Account created successfully.'); render();
}

function ensureDemoCertificate(){
  let cand = Object.values(state.candidates).find(c=>c.idNumber==='DEMO-JS-2026');
  if(!cand){
    cand = createCandidate({ fullName:'Joko Slamet', idNumber:'DEMO-JS-2026', institution:'Cipta Wacana University', department:'Official Testing Center' });
  }
  cand.fullName='Joko Slamet'; cand.institution='Cipta Wacana University'; cand.department='Official Testing Center'; cand.verified=true; cand.isDemo=true;
  cand.diagnostic={date:todayISO(),total:742,cefr:'C1',etp:etpStyle(742),readiness:91,weakSkills:['Listening','Speaking'],recommendations:['Advanced Listening Booster','Speaking & Intelligibility Studio','Full Mock Test Preparation']};
  cand.progress={orientation:true,listening:true,structure:true,reading:true,vocabulary:true,writing:true,speaking:true,mock:true};
  cand.mock={date:todayISO(),total:751,cefr:'C1',correct:11,totalItems:12};
  cand.official={sessionId:'JSPP-SESSION-DEMO-2026',date:todayISO(),total:765,cefr:'C1',etp:etpStyle(765),label:'Advanced academic user',skills:{Listening:150,Structure:160,Reading:165,Vocabulary:152,Writing:140,Speaking:148},integrity:'Verified',logs:[],strengths:['Reading','Structure'],recommendations:['Sustain academic listening accuracy','Continue speaking fluency development'],isDemo:true};
  const cert={id:'JSPP-CERT-SAMPLE',candidateId:cand.id,sessionId:cand.official.sessionId,score:765,cefr:'C1 Advanced',etp:etpStyle(765),integrity:'Verified',issueDate:todayISO(),expiryDate:String(new Date().getFullYear()+2)+'-'+todayISO().slice(5),status:'Valid',isDemo:true};
  state.certificates[cert.id]=cert; cand.certificates=[cert.id];
  state.batches[0].candidates=Object.keys(state.candidates).length; saveState(); return cand;
}

function quickCandidate(){
  const cand = ensureDemoCertificate();
...
function createCandidate(data){
  const id = uid('CAND'); const cand={ id, fullName:data.fullName || 'Candidate', idNumber:data.idNumber || uid('STU'), email:data.email || '', phone:data.phone || '', institution:data.institution || state.settings.institution || 'External Institution / Organization', department:data.department || '-', purpose:data.purpose || '', verified:true, progress:{}, certificates:[] };
