const CONFIG = {
  ...
  storageKey: 'js_proficiency_passport_ai_v7_diagnostic_audio_control',
  adminPin: 'JS2026',
...
const DIAG_ITEMS = (() => {
  ...
  const listeningScripts=[
    ['short campus conversation','Woman: Have you checked the new schedule? Man: Yes, the listening class has been moved to Friday morning. Narrator: What does the man mean?','the class schedule has changed','the class has been cancelled','the room is unavailable','the teacher is absent'],
    ['library service','Student: I need this article today. Librarian: You can access the digital copy through the database. Narrator: What does the librarian suggest?','use the online database','wait for a printed book','cancel the assignment','change the topic'],
    ['assignment deadline','Woman: I thought the essay was due next week. Man: The lecturer changed it to tomorrow. Narrator: What is implied?','the deadline is sooner than expected','the assignment was cancelled','the lecturer lost the essay','the class is optional'],
    ['academic lecture','Professor: Validity concerns whether a test measures what it is intended to measure. Narrator: What is the professor explaining?','the meaning of validity','the price of a test','the location of a test','the name of a textbook'],
    ['laboratory orientation','Staff: Please test your microphone before recording the speaking task. Narrator: What should students check?','their microphone','their printed certificate','their lunch schedule','their library card'],
    ['study strategy','Man: I listen to lectures twice and take notes the second time. Narrator: What strategy does the man use?','repeated listening with notes','ignoring lecture content','reading without listening','memorizing answers only'],
    ['seminar registration','Woman: Registration closes at noon, but payment can be made later. Narrator: What must be done first?','complete registration before noon','pay immediately before registering','attend without registration','email the certificate'],
    ['test preparation','Instructor: Do not focus only on grammar. Reading speed and listening inference also matter. Narrator: What is the instructor advising?','prepare several skills','study grammar only','avoid reading practice','skip listening tasks'],
    ['campus announcement','Announcement: The English proficiency test will begin at eight. Candidates should arrive thirty minutes earlier. Narrator: When should candidates arrive?','before eight o’clock','after the test begins','at noon','the next day'],
    ['feedback session','Lecturer: Your argument is clear, but your supporting evidence needs improvement. Narrator: What needs improvement?','supporting evidence','the topic title','the student ID','the classroom number'],
    ['academic talk','Professor: Inference questions require listeners to connect clues rather than repeat exact words. Narrator: What do inference questions require?','connecting clues','copying exact words only','checking the speaker’s name','counting paragraphs'],
    ['office consultation','Student: I cannot access the practice course. Admin: Reset your password and try again. Narrator: What does the admin recommend?','reset the password','delete the account','take the final test immediately','call the library']
  ];
  listeningScripts.forEach((x,i)=>items.push(['Listening',`Diagnostic Listening ${i+1}: Listen to the audio and answer the question.`,x[2],x[3],x[4],x[5],'Diagnostic Listening',`Narrator: Listen to a ${x[0]}. ${x[1]}`]));
  const structure=[
    ['The students ___ ready for the proficiency test.','are','is','be','being'],
    ['The instrument ___ by three experts before the pilot test.','was validated','validated','validating','has validating'],
    ['Neither the lecturer nor the students ___ late.','were','was','be','is'],
    ['The article ___ we discussed yesterday is useful.','that','what','who','where'],
    ['If the candidate studies regularly, she ___ improve her score.','will','would have','has','being'],
    ['The report needs ___ before submission.','revision','revise','revising it','to revising'],
    ['The researcher collected data ___ analyzed the results.','and','but also','because of','although'],
    ['The results are more reliable ___ the pilot sample is larger.','when','where','whose','what'],
    ['The lecturer asked the students ___ the form carefully.','to complete','complete','completed','completing'],
    ['The certificate can be verified ___ scanning the QR code.','by','for','to','from'],
    ['The test items, which were revised twice, ___ ready.','are','is','was','be'],
    ['A secure system prevents candidates from ___ other browsers.','opening','opened','open','to opening']
  ];
  structure.forEach((x,i)=>items.push(['Structure',`Diagnostic Structure ${i+1}: Choose the best answer. ${x[0]}`,x[1],x[2],x[3],x[4],'Diagnostic Structure']));
  const readingPassages=[
    'A diagnostic test is used before instruction to identify learners’ strengths and weaknesses. It should guide preparation rather than replace the official test.',
    'A secure online test requires identity checks, controlled access, item randomization, and review of suspicious behaviour. These layers help protect score meaning.',
    'CEFR levels describe what language users can do across contexts. They are useful for interpreting proficiency, but local validation remains important.',
    'Reading comprehension involves understanding main ideas, details, vocabulary in context, reference, and inference. Good readers connect information across sentences.',
    'Practice courses are most useful when they respond to learners’ needs. A learner who is weak in listening inference should receive listening-focused preparation.',
    'A verifiable certificate contains a unique code, candidate identity, score information, issue date, validity period, and a QR verification link.'
  ];
  for(let i=0;i<12;i++){
    const passage=readingPassages[i%readingPassages.length];
    const focuses=['main idea','detail','inference','vocabulary in context'];
    const focus=focuses[i%focuses.length];
    const correct={
      'main idea':'the central meaning of the passage',
      'detail':'a specific stated point from the passage',
      'inference':'a logical conclusion based on clues',
      'vocabulary in context':'the meaning of a word in the passage'
    }[focus];
    items.push(['Reading',`Diagnostic Reading ${i+1}: ${passage} Question focus: ${focus}.`,correct,'an unrelated opinion','a random grammatical rule','a personal biography','Diagnostic Reading']);
  }
  const vocab=[
    ['significant','important','ordinary','late','unclear'],['valid','supported','beautiful','silent','expensive'],['reliable','consistent','native-like','decorative','temporary'],['interpretation','meaning','building','schedule','password'],['evidence','supporting data','guess','decoration','absence'],['proficiency','language ability','test room','certificate paper','internet speed'],['diagnosis','identification of needs','final punishment','price reduction','random answer'],['integrity','honesty','noise','font style','travel plan'],['secure','protected','careless','open','informal'],['verify','confirm authenticity','delete records','hide results','change names'],['adaptive','responsive to level','fixed forever','printed only','unrelated'],['candidate','test taker','exam room','answer key','payment receipt']
  ];
  vocab.forEach((x,i)=>items.push(['Vocabulary',`Diagnostic Vocabulary ${i+1}: Choose the closest meaning of “${x[0]}”.`,x[1],x[2],x[3],x[4],'Diagnostic Vocabulary']));
  const writing=[
    ['A strong paragraph should have a clear topic sentence and ___.','supporting details','random spelling','only one word','no conclusion'],
    ['Coherence in writing means ideas are ___.','logically connected','hidden from readers','always translated','written without examples'],
    ['A thesis statement usually presents ___.','the main argument','the page number','the test password','the author’s age'],
    ['Academic writing should use evidence to support ___.','claims','seat numbers','login buttons','audio volume'],
    ['Revision is important because it helps writers ___.','improve clarity and accuracy','remove all ideas','avoid feedback','change identity'],
    ['Cohesive devices such as however and therefore help show ___.','relationships between ideas','student ID numbers','certificate borders','camera settings']
  ];
  writing.forEach((x,i)=>items.push(['Writing',`Diagnostic Writing ${i+1}: ${x[0]}`,x[1],x[2],x[3],x[4],'Diagnostic Writing']));
  const speaking=[
    ['In speaking assessment, intelligibility refers to how easily speech can be ___.','understood','printed','stored','deleted'],
    ['Fluency is related to the smoothness and continuity of ___.','speech','certificate design','question numbering','password typing'],
    ['A good oral response should answer the task and provide ___.','relevant support','random silence','only one sound','browser history'],
    ['Stress and intonation can help listeners understand ___.','meaning and emphasis','font color','page size','database name'],
    ['Speaking preparation can include recording, listening back, and ___.','improving delivery','copying certificates','opening new tabs','ignoring feedback'],
    ['In ELF-oriented assessment, the goal is clear communication rather than ___.','native-like accent only','honesty','evidence','test security']
  ];
  speaking.forEach((x,i)=>items.push(['Speaking',`Diagnostic Speaking ${i+1}: ${x[0]}`,x[1],x[2],x[3],x[4],'Diagnostic Speaking']));
  return items;
})();
const TEST_ITEMS = [
...
function renderDiagnostic(){
  const c=currentCandidate();
  return `<section class="section-title"><div><h2>AI Diagnostic Level Test</h2><p>Standard 60-item diagnosis across Listening, Structure, Reading, Vocabulary, Writing, and Speaking before practice and the real test.</p></div><button class="ghost" data-view="dashboard">Back to Dashboard</button></section>
  <div class="card mb"><h3>Diagnostic Blueprint</h3><p>60 items: Listening 12, Structure 12, Reading 12, Vocabulary 12, Writing 6, and Speaking 6. Listening diagnosis includes Play / Replay and Stop controls.</p></div>
...
function audioButton(script, prefix, i, isListening){
  ...
  const played=!!(state.realAudioPlayed&&state.realAudioPlayed[key]);
  if(isReal){
    return `<div class="audio-panel no-print"><button class="audio-btn speaker-btn ${played?'disabled':''}" type="button" data-action="playAudio" data-audio="${encoded}" data-key="${key}" ${played?'disabled':''} aria-label="Play listening item ${i+1} once"><span class="speaker-mark">🔊</span><span>${played?'Audio Already Played':'Play Audio Once'}</span></button><span class="small danger-text">Real-test listening is restricted to one play only.</span></div>`;
  }
  return `<div class="audio-panel no-print"><button class="audio-btn speaker-btn" type="button" data-action="playAudio" data-audio="${encoded}" data-key="${key}" aria-label="Play or replay listening item ${i+1}"><span class="speaker-mark">🔊</span><span>Play / Replay Audio</span></button><button class="audio-mini stop-audio" type="button" data-action="stopAudio" title="Stop audio">⏹ Stop</button><span class="small">Practice and diagnostic audio can be replayed and stopped.</span></div>`;
}
function quizItem(prefix,i,it){const isListening=it[0]==='Listening';const script=isListening?listeningScriptForItem(it,i):'';const realNote=(prefix==='r'&&isListening)?` <span class="pill red">1x audio only</span>`:'';return `<div class="quiz-item ${isListening?'listening-item':''}"><div class="question-line"><b>${i+1}. [${it[0]}]</b>${it[6]?` <span class="pill blue">${esc(it[6])}</span>`:''}${isListening?` <span class="pill gold">Speaker available</span>${realNote}`:''}</div>${audioButton(script,prefix,i,isListening)}<p>${esc(it[1])}</p><div class="options">${[2,3,4,5].map((idx,j)=>`<label class="option"><input type="radio" name="${prefix}${i}" value="${j===0?'correct':'wrong'}" required><span>${esc(it[idx])}</span></label>`).join('')}</div></div>`}
function renderITPItems(prefix, items){let last='';return items.map((it,i)=>{let header='';if(it[6]!==last){last=it[6];header=`<div class="section-break"><h3>${esc(it[6])}</h3><p>${it[0]==='Listening'?(prefix==='r'?'Each listening item has one clickable speaker and can be played only once in the real test.':'Listening audio can be played, replayed, and stopped in practice/mock/diagnostic mode.'):'Answer all questions in this section. Do not use outside materials.'}</p></div>`}return header+quizItem(prefix,i,it)}).join('')}
function stopAudio(){if('speechSynthesis' in window){window.speechSynthesis.cancel();toast('Audio stopped.')}}
function playAudio(encoded,key){const text=decodeURIComponent(encoded||'');if(!text)return toast('No audio script available.');if(!('speechSynthesis' in window))return toast('This browser does not support in-browser audio playback. Please use Safari, Chrome, or Edge with sound enabled.');const isReal=state.view==='realtest'&&key&&key.startsWith('r');if(isReal){state.realAudioPlayed=state.realAudioPlayed||{};if(state.realAudioPlayed[key]){toast('This listening item has already been played. Real-test audio is restricted to one play.');return;}state.realAudioPlayed[key]=true;save();document.querySelectorAll(`[data-key="${key}"]`).forEach(btn=>{btn.disabled=true;btn.classList.add('disabled');const span=btn.querySelector('span:last-child');if(span)span.textContent='Audio Already Played';});logSecurity(`Listening audio played once: ${key}`);}window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='en-US';u.rate=.86;u.pitch=1;u.volume=1;u.onstart=()=>toast(isReal?'Playing one-time real-test audio...':'Playing listening audio...');u.onerror=()=>toast('Audio playback was blocked. Click the speaker again or check browser sound permission.');window.speechSynthesis.speak(u)}
function renderPractice(){
  ...
  <div class="card mb"><h3>Listening Practice Lab</h3><p>Practice mode allows unlimited replay and stop controls before students enter the restricted real test.</p>${renderITPItems('p', makeRealTestItems().filter(x=>x[0]==='Listening').slice(0,12))}</div>
...
function renderMock(){
  ...
  <div class="card mb"><h3>Mock Test Blueprint</h3><p>15 original ITP-style demo items: Listening 5, Structure 5, Reading 5. Listening questions include Play / Replay and Stop controls.</p></div>
...
function renderRealTest(){
  ...
  <div class="card mb"><h3>Official-Style Test Directions</h3><p>This demo uses original institutional items and does not reproduce official TOEFL/ETS content. The structure follows the common ITP-style proportion: Listening Comprehension, Structure and Written Expression, and Reading Comprehension. In the real test, each listening item has a clickable speaker but can be played only once. Practice and mock modes allow replay and stop controls.</p></div>
...
function handleClick(e){
  ...
  if(action==='playAudio'){playAudio(b.dataset.audio,b.dataset.key);return}
