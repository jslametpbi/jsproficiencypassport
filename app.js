const CONFIG = {
  ...
  storageKey: 'js_proficiency_passport_ai_v4_final',
  adminPin: 'JS2026',
...
const TEST_ITEMS = [
  ['Listening','A student says, “I barely finished the reading before class.” What does the student imply?','The reading was difficult or time-consuming','The reading was very short','The class was cancelled','The lecturer did not assign reading'],
  ['Listening','In an academic talk, the phrase “on the other hand” signals ___.','contrast','definition','sequence only','example only'],
...
function quizItem(prefix,i,it){return `<div class="quiz-item"><b>${i+1}. [${it[0]}]</b> ${esc(it[1])}<div class="options">${[2,3,4,5].map((idx,j)=>`<label class="option"><input type="radio" name="${prefix}${i}" value="${j===0?'correct':'wrong'}" required><span>${esc(it[idx])}</span></label>`).join('')}</div></div>`}
...
function renderMock(){
  const c=currentCandidate();
  return `<section class="section-title"><div><h2>Mock Test</h2><p>Medium-security preparation test. This is not the official certificate test.</p></div><button class="ghost" data-view="dashboard">Back to Dashboard</button></section>
  <form id="mockForm" class="card">${TEST_ITEMS.slice(0,8).map((it,i)=>quizItem('m',i,it)).join('')}<button class="primary" type="submit">Submit Mock Test</button></form>
...
function renderRealTest(){
  ...
  <section class="section-title"><div><h2>Secure Real Test</h2><p>Strict mode: fullscreen request, copy/paste/right-click restriction, tab-switch logs, dynamic watermark, and integrity review.</p></div><button class="ghost" data-view="dashboard">Back to Dashboard</button></section>
  <div class="secure-bar no-print"><span class="pill gold">Candidate: ${esc(c.name)}</span><span class="pill blue">Code: ${esc(c.registrationCode)}</span><span class="pill green">Camera/Mic Check: Demo Ready</span><span class="pill red">Timer: <b id="timer" class="timer">30:00</b></span><button class="primary" data-action="fullscreen">Enter Fullscreen</button></div>
  <form id="realTestForm" class="card test-shell">${TEST_ITEMS.map((it,i)=>quizItem('r',i,it)).join('')}<div class="field"><label>Writing Task</label><textarea name="writing" placeholder="Write a short academic response about the importance of secure English proficiency testing." required></textarea></div><button class="primary mt" type="submit">Submit Official Test</button></form>
  <div class="card mt"><h3>Security Log</h3><div class="list">${logs.length?logs.map(l=>`<div class="log"><span>${esc(l.type)}</span><small>${esc(l.time)}</small></div>`).join(''):'<p class="small">No violation logged in this session.</p>'}</div></div>`;
}
...
function submitMock(fd){let correct=0;TEST_ITEMS.slice(0,8).forEach((_,i)=>{if(fd.get('m'+i)==='correct')correct++});const score=320+Math.round(correct/8*430);const c=currentCandidate();c.mock={score,date:today()};save();toast(`Mock test completed: ${score}/900.`);navTo('report')}
function submitReal(fd){let correct=0;TEST_ITEMS.forEach((_,i)=>{if(fd.get('r'+i)==='correct')correct++});const writing=String(fd.get('writing')||'').trim();const writingBonus=Math.min(120,Math.round(writing.length/6));const score=Math.min(900,380+Math.round(correct/TEST_ITEMS.length*370)+writingBonus);const c=currentCandidate();const logs=c.realTest?.securityLogs||[];c.realTest={score,date:today(),integrity:logs.length?'Review Required':'Verified',securityLogs:logs};makeCertificate(c,false);save();toast(`Official test submitted: ${score}/900. Certificate generated.`);detachSecurity();navTo('certificate')}
