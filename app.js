const CONFIG = {
  ...
  storageKey: 'js_proficiency_passport_ai_v6_all_listening_audio',
  adminPin: 'JS2026',
...
function audioButton(script, prefix, i, isListening){
  ...
  return `<div class="audio-panel no-print"><button class="audio-btn speaker-btn" type="button" data-action="playAudio" data-audio="${encodeURIComponent(safeScript)}" data-key="${prefix}${i}" aria-label="Play or relisten to listening item ${i+1}"><span class="speaker-mark">🔊</span><span>Play / Relisten Audio</span></button><button class="audio-mini" type="button" data-action="playAudio" data-audio="${encodeURIComponent(safeScript)}" data-key="${prefix}${i}-mini" title="Speaker">🔈</button><span class="small">Audio ready for this listening item.</span></div>`
}
function quizItem(prefix,i,it){const isListening=it[0]==='Listening';const script=isListening?listeningScriptForItem(it,i):'';return `<div class="quiz-item ${isListening?'listening-item':''}"><div class="question-line"><b>${i+1}. [${it[0]}]</b>${it[6]?` <span class="pill blue">${esc(it[6])}</span>`:''}${isListening?` <span class="pill gold">Speaker available</span>`:''}</div>${audioButton(script,prefix,i,isListening)}<p>${esc(it[1])}</p><div class="options">${[2,3,4,5].map((idx,j)=>`<label class="option"><input type="radio" name="${prefix}${i}" value="${j===0?'correct':'wrong'}" required><span>${esc(it[idx])}</span></label>`).join('')}</div></div>`}
function renderITPItems(prefix, items){let last='';return items.map((it,i)=>{let header='';if(it[6]!==last){last=it[6];header=`<div class="section-break"><h3>${esc(it[6])}</h3><p>${it[0]==='Listening'?'Every listening item below has a clickable speaker button for Play / Relisten audio.':'Answer all questions in this section. Do not use outside materials.'}</p></div>`}return header+quizItem(prefix,i,it)}).join('')}
function playAudio(encoded){const text=decodeURIComponent(encoded||'');if(!text)return toast('No audio script available.');if(!('speechSynthesis' in window))return toast('This browser does not support in-browser audio playback. Please use Safari, Chrome, or Edge with sound enabled.');window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='en-US';u.rate=.86;u.pitch=1;u.volume=1;u.onstart=()=>toast('Playing listening audio...');u.onerror=()=>toast('Audio playback was blocked. Click the speaker again or check browser sound permission.');window.speechSynthesis.speak(u);if(state.view==='realtest')logSecurity('Listening speaker audio played/replayed')}
