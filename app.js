      ...
      <section class="hero premium-landing">
        <div class="hero-panel premium-hero">
          ...
          <div>
            ...
            <div class="landing-preview-grid mt">
              <div class="verify-card">
                <h3>Certificate Verification <span class="badge green">Demo</span></h3>
                <div class="verify-row"><span>Certificate ID</span><strong>JSPP-CERT-SAMPLE</strong></div>
                <div class="verify-row"><span>Candidate Name</span><strong>Joko Slamet</strong></div>
                <div class="verify-row"><span>Status</span><strong class="valid-text">Valid</strong></div>
                <div class="verify-row"><span>Overall Score</span><strong>765 / 900</strong></div>
                <div class="verify-row"><span>CEFR Level</span><strong>C1 Advanced</strong></div>
                <button class="primary verify-btn" data-action="showVerifyHelp">▦ Verify with QR</button>
              </div>
              <div class="certificate-mini">
                <div class="mini-demo-watermark">DEMO</div>
                <h4>CERTIFICATE OF ENGLISH PROFICIENCY</h4>
                <p>This is to certify that</p>
                <strong>Joko Slamet</strong>
                <p class="demo-under-name">Demo / example certificate</p>
                <div class="mini-score"><span>765 / 900</span><span>C1 Advanced</span></div>
                <p>Issued by<br/><b>Cipta Wacana University</b></p>
                <div class="mini-qr">QR</div>
                <div class="mini-seal">VALID</div>
              </div>
            </div>
          </div>
        ...
        <aside class="login-card access-card" id="registerPanel">
          <div class="access-head">
            ...
            <div><h2>Secure Access</h2><p>Sign in to your account or create a new one.</p></div>
            <div class="access-tabs"><button class="ghost" type="button">Sign In</button><button class="primary" type="button">Register</button></div>
          </div>
          <form id="loginForm" class="form registration-form">
            <div class="field full"><label>Select Your Role</label>
              <div class="role-grid">
                <label class="role-card selected"><input type="radio" name="role" value="candidate" checked /><span>♙</span><strong>Candidate<br/>Test Taker</strong></label>
                <label class="role-card"><input type="radio" name="role" value="admin" /><span>⬟</span><strong>Admin</strong></label>
            ...
            <div class="field"><label>Full Name</label><input name="fullName" placeholder="Enter your full name" required /></div>
            ...
            <div class="field"><label>Email Address</label><input name="email" type="email" placeholder="Enter your email address" /></div>
            <div class="field"><label>Institution / Organization</label><input name="institution" placeholder="Enter institution or organization" /></div>
            <div class="field"><label>Department / Unit</label><input name="department" placeholder="Enter department or unit" /></div>
            <div class="field"><label>Password</label><input name="password" type="password" placeholder="Create a strong password" /></div>
            <div class="field"><label>Confirm Password</label><input name="confirmPassword" type="password" placeholder="Confirm your password" /></div>
            ...
            <button class="primary register-submit full" type="submit">▣ Enter / Create Account</button>
            <div class="secure-note full"><span>⬟</span><p>Your information is protected for official testing and institutional use. Candidate demo uses local browser storage; production deployment should use encrypted authentication and protected databases.</p></div>
          </form>
        </aside>
      </section>
    </main>`;
...
document.addEventListener('click', async (e)=>{
  ...
  if(action==='focusRegister'){ document.getElementById('registerPanel')?.scrollIntoView({behavior:'smooth',block:'center'}); document.querySelector('input[name="fullName"]')?.focus(); return; }
...
function handleLogin(form){
  ...
  if(fd.get('password') && fd.get('confirmPassword') && fd.get('password') !== fd.get('confirmPassword')){ toast('Password confirmation does not match.'); return; }
...
function createCandidate(data){
  const id = uid('CAND'); const cand={ id, fullName:data.fullName || 'Candidate', idNumber:data.idNumber || uid('STU'), institution:data.institution || state.settings.institution || 'External Institution / Organization', department:data.department || '-', verified:true, progress:{}, certificates:[] };
